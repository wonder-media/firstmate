...
- Helper scripts in `bin/` are plain bash, except the captain dashboard: `bin/fm-board.py` and the `bin/board/` helpers are Python 3.14 standard library only, and `bin/fm-board.sh`'s header owns how that interpreter is selected.
