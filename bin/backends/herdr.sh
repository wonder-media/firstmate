...
fm_backend_herdr_cli_exec() {  # <session> <client-bin> <herdr-args...>
  local session=$1 client_bin=$2
  shift 2
  if [ -n "${FM_BACKEND_HERDR_CALL_TIMEOUT:-}" ]; then
    fm_run_timed "$FM_BACKEND_HERDR_CALL_TIMEOUT" \
      env HERDR_SESSION="$session" "$client_bin" "$@" --session "$session"
  else
...
fm_backend_herdr_pane_process_state_sample() {  # <session> <pane_id>
  ...
  info=$(fm_backend_herdr_read_cli "$session" pane process-info --pane "$pane_id" 2>/dev/null) \
