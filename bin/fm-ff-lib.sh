...
dirty_status() {
  ...
  if [ "$ignore_seed_marker" = yes ]; then
    git -C "$dir" status --porcelain 2>/dev/null |
      awk -v marker="?? $SUB_HOME_MARKER" -v owned="$FM_FF_HOME_OWNED_UNTRACKED_RE" \
        '$0 != marker && $0 !~ owned { print; exit }'
  else
...
ff_target() {
  ...
  local secondmate_id=${6:-} reconciliation_state=${7:-} local_source=${8:-}
  FF_STATUS="skipped"
  FF_INSTR=""
...
process_secondmate() {
  ...
  if [ "$base_mode" = origin ]; then
    ...
    ff_target "$home_real" "secondmate $id" "$base_mode" yes yes "$id" "${FM_STATE_OVERRIDE:-$FM_HOME/state}" "$FM_ROOT"
  fi
