...
#          bin/fm-tool-versions-lib.sh owns the support floors. An installed
...
#          bin/fm-version-inventory.sh owns the separate opt-in freshness
...
# shellcheck source=bin/fm-tool-versions-lib.sh disable=SC1091
...
# shellcheck source=bin/fm-secondmate-dormant-lib.sh disable=SC1091
...
secondmate_liveness_sweep() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    if fm_secondmate_dormant_present "$STATE" "$id"; then
      [ "${FM_BOOTSTRAP_VERBOSE_FACTS:-0}" != 1 ] || echo "BOOTSTRAP_INFO: secondmate $id is dormant (expected stopped state)"
      continue
    fi
    remote_host=$(fm_meta_get "$meta" remote_host)
    label=$id
...
if [ "${1:-}" = "lavish-compatible" ]; then
  tool_version_at_least lavish-axi "$FM_LAVISH_AXI_MIN"
  exit
fi
...
detect_local_tools() {
  ...
  if fm_backend_list_contains "$TOOLS" treehouse \
    ...
    && ! fm_treehouse_help_declares_lease "$(treehouse get --help 2>&1)"; then
    echo "MISSING: treehouse (install: $(install_cmd treehouse))"
  fi
  if command -v no-mistakes >/dev/null 2>&1 && ! tool_version_at_least no-mistakes "$FM_NO_MISTAKES_MIN"; then
    echo "MISSING: no-mistakes (install: $(install_cmd no-mistakes))"
  fi
  if command -v gh-axi >/dev/null 2>&1 && ! tool_version_at_least gh-axi "$FM_GH_AXI_MIN"; then
    echo "MISSING: gh-axi (install: $(install_cmd gh-axi))"
  fi
  if ! tool_version_at_least lavish-axi "$FM_LAVISH_AXI_MIN"; then
    echo "PRESENTATION_UNAVAILABLE: lavish-axi (requires >=$FM_LAVISH_AXI_MIN; install: $(install_cmd lavish-axi)) - nonvisual work may proceed with plain-text decisions and reports; install or upgrade before using Lavish"
  fi
