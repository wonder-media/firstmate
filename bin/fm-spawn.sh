...
launch_template() {
  ...
  codex)
    if [ "$kind" = secondmate ]; then
      ...
      printf '%s' 'codex __MODELFLAG____EFFORTFLAG____AUTOCOMPACTFLAG__--dangerously-bypass-approvals-and-sandbox --disable hooks -c "notify=[\"bash\",\"-c\",\"touch __TURNEND__\"]" "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
    fi
    ;;
...
autocompact_flag_for_harness() {
  ...
  *%)
    ...
    case "$harness" in
    codex) tokens=$(codex_percent_autocompact_tokens "$percent" "$model") || return 0 ;;
    claude)
...
if [ "$KIND" = secondmate ]; then
  ...
  # standalone clone may acquire only that exact object from this local primary.
  # The same guarded path can reconcile a clean divergence already present at
  # the target; a dirty, uniquely diverged, or wrong-branch home is left untouched
  # and launches as-is. The agent re-reads
  ...
  if [ "${FM_SKIP_SECONDMATE_SYNC:-0}" = 1 ]; then
    ...
    sm_ff_out=$(ff_target "$PROJ_ABS" "secondmate $ID" "$sm_primary_head" yes yes "$ID" "$STATE" "$FM_ROOT" 2>&1 || true)
    case "$sm_ff_out" in
    *': skipped:'*)
