...
#     the full canonical set: bin/*.sh bin/backends/*.sh bin/board/*.sh
#     tests/*.sh. This is what CI always runs, so CI coverage never depends
#     on a local diff.
...
# *.sh child of bin/, bin/backends/, bin/board/, or tests/) without the shell case
...
fm_lint_is_canonical_root() {
  ...
  case "$dir" in
    bin|bin/backends|bin/board|tests) return 0 ;;
    *) return 1 ;;
  esac
}
...
if [ "$#" -gt 0 ]; then
  ...
  if [ "$full_lint" -eq 1 ]; then
    ROOTS=(bin/*.sh bin/backends/*.sh bin/board/*.sh tests/*.sh)
  else
    CHANGED_MODE=1
    ROOTS=()
