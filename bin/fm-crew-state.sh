...
# heartbeat:
#
#   state: <working|idle|parked|done|blocked|paused|failed|unknown> · source: <run-step|pane|status-log|remote-endpoint|none> · <detail>
#
# Logic, in order:
...
target_presence() {  # <target> -> present|gone|unreadable
  ...
  herdr)
    case "$(fm_backend_target_presence "$TASK_BACKEND" "$1" "$EXPECTED_LABEL")" in
...
if [ "$KIND" = secondmate ]; then
  emit_secondmate_declared_or() {  # [<fallback-state> <fallback-source> [detail]]
    ...
    paused) emit paused status-log "$(status_line_note "$LOG_LINE")" ;;
    blocked)
  ...
  busy) emit working pane "secondmate coordinator active (${BUSY_VERDICT#* })" ;;
  idle)
...
*)
  if [ "$TASK_BACKEND" = herdr ]; then
    ...
    case "$AGENT_STATE" in
    missing) emit unknown none "backend target gone: $BACKEND_TARGET" ;;
    dead) emit unknown none "backend target gone: $BACKEND_TARGET (agent gone, pane shell remains)" ;;
    esac
  fi
