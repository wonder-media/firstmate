...
family_for_basename() {
  case "$1" in
    ...
    fm-tangle-guard.test.sh|fm-update.test.sh|fm-version-inventory.test.sh)
      printf '%s\n' session-bootstrap
      ;;
...
run_script_bounded() {  # <script> <out> <stream> <id>
  ...
  if [ "$stream" -eq 1 ]; then
    if [ "$PER_SCRIPT_TIMEOUT_SECS" -gt 0 ]; then
      ...
      fm_test_env_run "$RUN_TMP/env.$id" fm_run_timed "$PER_SCRIPT_TIMEOUT_SECS" bash -c \
        'bash "$1" 2>&1 | tee "$2"; exit "${PIPESTATUS[0]}"' _ "$script" "$out"
      rc=$?
    else
      # Expansion is intentionally deferred to the child bash passed to -c.
      # shellcheck disable=SC2016
      fm_test_env_run "$RUN_TMP/env.$id" bash -c \
        'bash "$1" 2>&1 | tee "$2"; exit "${PIPESTATUS[0]}"' _ "$script" "$out"
      rc=$?
    fi
  elif [ "$PER_SCRIPT_TIMEOUT_SECS" -gt 0 ]; then
    fm_test_env_run "$RUN_TMP/env.$id" fm_run_timed "$PER_SCRIPT_TIMEOUT_SECS" \
      bash "$script" >"$out" 2>&1
    rc=$?
  else
    fm_test_env_run "$RUN_TMP/env.$id" bash "$script" >"$out" 2>&1
    rc=$?
  fi
