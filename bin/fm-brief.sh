...
When you raise a keyed `needs-decision` with clear alternatives, first register 2-3 concrete options with `__FM_ROOT__/bin/fm-board.sh decision`, including an ELI5 title, a one-sentence consequence, the recommendation, and its reason.
...
Then append `needs-decision [key=deploy-window] [at=<epoch>]: Choose when to release` and stop as required below.
EOF
...
EOF
WORKER_RECORD_RULES=${WORKER_RECORD_RULES%$'\n'}

# One shared string keeps the ship and scout evidence boundary identical.
IFS= read -r -d '' EVIDENCE_DISCIPLINE_RULE <<'EOF' || true
...
EOF
EVIDENCE_DISCIPLINE_RULE=${EVIDENCE_DISCIPLINE_RULE%$'\n'}
