...
| `fm-test-env-lib.sh`     | Shared test-process environment sanitization and synthetic-home assertions |
...
| `fm-tool-versions-lib.sh` | Shared compatibility floors, release-channel identities, and intentional CI pins    |
| `fm-lock-lib.sh`         | Shared "is this git lock provably abandoned?" proof used by teardown and fleet-sync   |
| `fm-config-inherit-lib.sh` | Shared primary-to-secondmate inherited local-material propagation and config-reread delivery |
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend selector and `tasks-axi` compatibility probe                  |
| `fm-quota-axi-lib.sh`    | Shared `quota-axi` compatibility probe for the bootstrap diagnostic                  |
...
| `fm-control.sh`          | Agent lifecycle control plane: exact-id allowlisted verbs, including persistent-secondmate dormancy and wake ([agent-control.md](agent-control.md)) |
| `fm-control-lib.sh`      | One executable owner of the control-plane verb allowlist, per-harness interrupt/exit mechanics, per-backend capability, and per-task harness wiring installation and retirement |
| `fm-secondmate-dormant-lib.sh` | Shared durable dormant-record path, write, presence, and clear helpers for persistent secondmates |
...
| `fm-pr-check.sh`         | Record PR metadata, arm a static merge poll, and mark the task as waiting for the captain's merge |
| `fm-pr-merge.sh`         | Record PR metadata, then merge a task's canonical full GitHub URL                    |
| `fm-github-app-token.sh` | Run one allowlisted `gh`/`gh-axi` PR or release command under the opt-in GitHub App identity ([configuration.md](configuration.md#github-app-automation-configgithub-app-credentials)) |
