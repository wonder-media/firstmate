...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, atomically arm a static merge poll, and mark the task as waiting for the captain's merge; refuses a GitHub draft |
