...
| `fm-control.sh`          | Exact-id lifecycle control, including persistent-secondmate dormancy and wake ([agent-control.md](agent-control.md)) |
| `fm-control-lib.sh`      | One executable owner of control-plane mechanics, endpoint proofs, and per-task harness wiring installation and retirement |
| `fm-secondmate-dormant-lib.sh` | Durable dormant-record path, write, presence, and clear helpers for persistent secondmates |
...
| `fm-pr-check.sh`         | Record validated task `pr=` and `pr_head=` values, atomically arm a static merge poll, and mark the task as waiting for the captain's merge; refuses GitHub drafts and persistent secondmate records (see [architecture.md](architecture.md)) |
