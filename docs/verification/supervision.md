...
Session-lock ownership in `bin/fm-session-lock-lib.sh` is decided against the eligible session processes in a contiguous harness ancestry rather than one chosen depth.
Claude `daemon run`, `bg-pty-host`, and `bg-spare` processes are traversal-only infrastructure and are never emitted as candidates or accepted by the live-owner predicate.
A lock whose only live match is one of those processes is stale, and an ancestry containing only those processes cannot acquire the lock.
...
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table and runs the real Stop auto-arm in version-named, harness-named-parent, and combined real process trees.
`tests/fm-claude-stop-autoarm.test.sh` drives the public `fm-lock.sh` status and acquire paths against real fake-Claude daemon and background-helper processes.
