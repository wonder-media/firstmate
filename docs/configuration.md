...
Direct secondmate coordinator state is supported for Claude, OpenCode, Pi, and pi-signed on the state-verified tmux and Herdr backends. `bin/fm-crew-state.sh` owns the proof and state mapping; `bin/fm-control-lib.sh` owns installation and retirement of lifecycle wiring. Claude's hooks are guest-merged into captain-owned settings when `jq` and parseable JSON are available; otherwise the mate launches unwired and reports `unknown` without rewriting that file.
...
For Pi and pi-signed secondmate launches, `fm-spawn.sh` passes the state-resident semantic lifecycle extension when armed, plus the secondmate home's tracked `.pi/extensions/fm-primary-pi-watch.ts` and `.pi/extensions/fm-primary-turnend-guard.ts` primary extensions.
...
The file is primary-authoritative inherited local material; `fm_config_inherit_items` in `bin/fm-config-inherit-lib.sh` owns the complete declared set propagated to secondmate homes.
...
Lavish is optional for nonvisual work, but visual use requires the compatible version declared with the other tool floors.
[`bin/fm-tool-versions-lib.sh`](../bin/fm-tool-versions-lib.sh) owns the support floors and exact release-channel identities.
...
The local route's linked-worktree and standalone-clone convergence contract is owned by [`secondmate-provisioning`](../.agents/skills/secondmate-provisioning/SKILL.md); remote routes retain their configured-host contract.
...
FM_SNAPSHOT_RUN_LOOKUP_TIMEOUT=2   # seconds per no-mistakes query inside one fm-fleet-snapshot.sh current-state read
FM_SNAPSHOT_CREW_STATE_TIMEOUT=    # whole bound per fm-fleet-snapshot.sh current-state read; defaults to 2 run lookups + 3 Herdr endpoint reads + 1s; remote endpoint liveness is not probed on the snapshot path
...
FM_BACKEND_HERDR_READ_TIMEOUT=3   # seconds per passive Herdr endpoint read; timeout is unknown, never absent or idle
...
A Hold on work the stored backlog already reports done or no longer lists is refused with a Conflict and the card refreshes, so completed work is never reopened or relaunched.
Hold adds a parked `tasks-axi` hold only when the task records none; an existing captain, external, or dated hold keeps its kind, reason, and date even when expired, and Resume releases only Bridge's own hold and relaunches only the exact worker Bridge stopped, meaning `fm-control.sh exit` reported `stopped` rather than `already-stopped`.
When a recorded worker was not stopped by Bridge, or the stopped worker's record changed or vanished, Resume completes only if the backend's recovery-grade agent classifier reports the current agent alive; otherwise, and when a relaunch or worker stop fails, the work stays in the Archive with the error until a retried request succeeds, and neither ingest nor queueing or undoing that retry clears the error on its own.
Each failed Hold, Discard, or Resume request also exports one request-scoped packet (`request_id`, `home`, `task`, `lifecycle` action, and `error`, never an `answer_id` or `choice`) through the `board-answers` source below, so the owning coordinator is woken with durable context; the packet stays an exception until the task no longer carries that error.
Discard declines each open captain decision hold of the task through `fm-decision-hold.sh` with a decision file that records the Bridge request, so the owner's dependency guard still applies and a refusal stays visible in the Archive.
Ingest treats `tasks-axi` as the authority for held and discarded work with no request in flight and no failed step: a task finished elsewhere leaves the Archive, and a task released or reopened elsewhere returns to the active views, while a task whose Hold or Discard failed keeps its Archive attention card and error even after it is released or finished elsewhere.
Set `FM_BOARD_BROWSER_TEST=1` when running `tests/fm-board.test.sh` to add rendered Chrome assertions at a desktop and a phone window size, each reported separately; `FM_BOARD_BROWSER_SIZE=<width>,<height>` renders one operator-chosen size instead. The test prints an explicit skip when the flag or a Chrome binary is absent, while API, SSE, and answer-lifecycle checks always run.
