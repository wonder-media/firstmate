...
The same deferred stage runs bootstrap's guarded secondmate sync for recorded live homes, then propagates declared inherited local material into each validated live home.
...
FM_BACKEND_HERDR_READ_TIMEOUT=3    # seconds per passive Herdr endpoint read (pane get, agent get); a timeout reads as unknown, never as absent
FM_SNAPSHOT_CREW_STATE_TIMEOUT=    # whole bound per fm-fleet-snapshot.sh current-state read; defaults to 2 run lookups + 3 endpoint reads + 1s
