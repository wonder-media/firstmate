...
Direct secondmate coordinator state is supported for Claude, OpenCode, Pi, and pi-signed on the state-verified tmux and Herdr backends.
`bin/fm-crew-state.sh` owns the proof and state mapping; `bin/fm-control-lib.sh` owns installation and retirement of lifecycle wiring.
Claude's hooks are guest-merged into captain-owned settings when `jq` and parseable JSON are available; otherwise the mate launches unwired and reports `unknown` without rewriting that file.
...
Visual use requires the compatible Lavish version declared with the other tool floors.
