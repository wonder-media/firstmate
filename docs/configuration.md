...
Hold adds a parked `tasks-axi` hold only when the task has none; an existing captain, external, or dated hold and its reason stay untouched, and Resume releases only Bridge's own hold and relaunches only the exact worker Bridge stopped, keeping the work held with a visible error when that relaunch fails.
