...
When the stopped worker's record changed or vanished, Resume completes only if the backend's recovery-grade agent classifier reports the current agent alive; otherwise, and when a relaunch or worker stop fails, the work stays in the Archive with the error until a retried request succeeds, and ingest never clears that error on its own.
