...
Codex's primary watcher protocol is `bin/fm-watch-checkpoint.sh --seconds "${FM_CODEX_WATCH_CHECKPOINT:-300}"`, not `bin/fm-watch-arm.sh`.
...
The state-resident extension is written only when the semantic lifecycle wiring is armed, which for a secondmate means a state-verified backend (tmux or Herdr); when it is not armed that first `-e` flag is dropped from the launch instead of naming a file firstmate never wrote, so the launch carries the other two.
