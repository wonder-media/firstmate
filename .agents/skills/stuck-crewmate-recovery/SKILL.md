...
Run `treehouse status` from the task's recorded `project=` clone in a subshell, because Treehouse reports only the pool of the repository at its working directory, and the firstmate home shows a different pool or none.
