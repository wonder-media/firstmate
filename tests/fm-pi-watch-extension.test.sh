...
// The event handler does not await its own arm attempt, and that attempt walks
// the process ancestry with ps. Await the coordinator instead of a fixed sleep:
// it settles the in-flight attempt, so the assertion never races the walk.
