...
test_five_herdr_secondmates_isolate_one_timed_out_endpoint() {
  ...
  # its three read_bound Herdr reads. Even if the five reads ran in sequence,
  # the wall-clock ceiling is the sum of those bounds plus a fixed headroom for
...
test_completed_scout_report_is_pointer_not_pending() {
  local home fakebin out kind terminal id phase single mate single_state mate_state fixture_gen
  ...
  for kind in ship scout secondmate; do
    for terminal in 'done' failed; do
      id="$kind-$terminal"
      fixture_gen=$("$ROOT/bin/fm-busy-event.sh" arm "$home/state" "$id")
      "$ROOT/bin/fm-busy-event.sh" apply "$home/state" "$id" idle --gen "$fixture_gen" \
        --source claude-hook --event stop
      fm_write_meta "$home/state/$id.meta" \
        "window=firstmate:fm-$id" "worktree=$home/projects/task" \
        "kind=$kind" "harness=claude" "busy_gen=$fixture_gen"
