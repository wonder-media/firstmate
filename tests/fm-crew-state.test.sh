...
reset_fakes() {
  ...
  export FM_FAKE_HERDR_BUSY FM_FAKE_HERDR_MISSING FM_FAKE_HERDR_READ_FAIL FM_FAKE_HERDR_HUSK FM_FAKE_HERDR_HANG FM_FAKE_HERDR_AGENT_STATUS FM_FAKE_HERDR_PROCESS FM_FAKE_HERDR_SHELL_PID FM_FAKE_CI_LOGS
...
SH
  ...
  assert_contains "$out" "backend target unreadable" "a failed herdr CLI must read as unreadable, not gone"
...
test_newest_open_decision_supplies_the_reported_detail() {
  reset_fakes
  local d out
  d=$(make_secondmate_lifecycle_case newest-open-decision claude claude-hook idle tmux)
...
ROWS
  ...
  out=$(run_crew_state "$d" mate)
  assert_contains "$out" "state: working" "live busy proof outranks a declaration"
  ...
  out=$(run_crew_state "$d" mate)
  assert_contains "$out" "state: parked" "idle lifecycle plus open decision -> parked"
  ...
  out=$(run_crew_state "$d" mate)
  assert_contains "$out" "state: paused" "declared pause outranks healthy idle"
  ...
  reset_fakes
  d=$(new_case secondmate-unsupported)
  mkdir -p "$d/wt"
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/mate.meta" "window=fm:fm-mate" "endpoint_task_id=mate" \
    "worktree=$d/wt" "project=$d/wt" "kind=secondmate" "harness=cursor" "home=$d/wt"
  printf 'blocked: waiting on vendor\n' > "$d/state/mate.status"
  out=$(run_crew_state "$d" mate)
  assert_contains "$out" "state: blocked" "declared blocker survives unsupported lifecycle wiring"

  pass "secondmate coordinator state uses endpoint, generation, lifecycle, decisions, and declarations"
}
...
test_secondmate_herdr_read_timeout_is_bounded_unknown() {
  ...
  start=$SECONDS
  out=$(PATH="$d/fakebin:$PATH" FM_STATE_OVERRIDE="$d/state" FM_BACKEND_HERDR_READ_TIMEOUT=1 "$CREW_STATE" mate)
  elapsed=$((SECONDS - start))
  assert_contains "$out" "state: unknown" "a timed-out endpoint must not invent current state"
  assert_contains "$out" "endpoint state unavailable (unreadable)" "the endpoint timeout is disclosed as unreadable"
  [ "$elapsed" -lt 4 ] || fail "one Herdr endpoint read exceeded its short bound (${elapsed}s)"
  pass "a blocked Herdr endpoint read is bounded and degrades to disclosed unknown"
}
...
test_no_run_herdr_presence_distinguishes_gone_from_unreadable() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-herdr-presence.meta" "window=default:w1:p9" "worktree=$d/wt" "kind=ship" "backend=herdr" "harness=claude"
  FM_FAKE_HERDR_MISSING=1
  out=$(run_crew_state "$d" feat-herdr-presence)
  assert_contains "$out" "backend target gone: default:w1:p9" "pane_not_found is disclosed as gone"
  FM_FAKE_HERDR_MISSING=0
  FM_FAKE_HERDR_HANG=1
  start=$SECONDS
  out=$(FM_BACKEND_HERDR_READ_TIMEOUT=1 run_crew_state "$d" feat-herdr-presence)
  elapsed=$((SECONDS - start))
  assert_contains "$out" "backend target unreadable: default:w1:p9" "timeout is disclosed as unreadable"
  assert_not_contains "$out" "backend target gone" "timeout must not claim the target is gone"
  [ "$elapsed" -lt 4 ] || fail "presence read exceeded its short bound (${elapsed}s)"
  pass "a Herdr crew distinguishes gone from unreadable"
}
