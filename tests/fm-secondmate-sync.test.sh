...
test_remote_sync_ignores_firstmate_lifecycle_artifacts() {
  ...
  assert_contains "$out" "dirty working tree" "a real uncommitted edit must still refuse the remote sync"
...
test_sweep_skips_dormant_home_silently_unless_verbose() {
  ...
  assert_contains "$out" "BOOTSTRAP_INFO: secondmate sm is dormant; convergence is deferred until wake" \
    "verbose facts should name dormant convergence debt"
  [ "$(head_of "$w/sm")" = "$before" ] || fail "verbose reporting converged a dormant home"
  pass "T3f dormant: sweep defers convergence until wake"
}
