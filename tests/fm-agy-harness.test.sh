...
# which runners do not keep in the system bin dirs. Expose only node through a
# dedicated shim directory: the real node directory may also contain agy,
# which would invalidate the missing-executable case.
NODE_BIN=$(command -v node) || fail "test needs node"
NODE_BIN_DIR=$(fm_fakebin "$TMP_ROOT/node-bin")
ln -s "$NODE_BIN" "$NODE_BIN_DIR/node"
