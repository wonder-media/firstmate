...
| `/1by1`            | Walk fresh items needing captain feedback one at a time: each response presents one decision, approval, review request, or human prerequisite with 2-3 options plus Hold, Discard, Skip, and Stop, waits for your answer, and records it through the existing owner; add a project selector such as `/1by1 wok` to isolate one project |
